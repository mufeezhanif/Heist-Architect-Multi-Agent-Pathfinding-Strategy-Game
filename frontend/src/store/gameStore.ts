...
export interface GameState {
  ...
  narrationEntries: {
    text: string;
    type: "move" | "sensor" | "warden" | "objective" | "alert" | "info";
  }[];
  ...
  setExecutionMode: (
    mode: "idle" | "stepping" | "playing" | "paused" | "done",
  ) => void;
  ...
  create<GameState>()((set) => ({
    ...
    setTurnResult: (r) =>
      set({
        turnResult: r,
        turn: r.turn,
        gameStatus: (r.game_status === "planning"
          ? "active"
          : r.game_status) as "active" | "won" | "lost",
        score: r.score,
    ...
    addEventLog: (msg) =>
      set((s) => ({ eventLog: [...s.eventLog.slice(-19), msg] })),
    ...
    pushStepToQueue: (step) =>
      set((s) => ({ stepQueue: [...s.stepQueue, step] })),
    popStepFromQueue: () => {
    ...
    addNarration: (entry) =>
      set((s) => ({
        narrationEntries: [...s.narrationEntries.slice(-29), entry],
      })),
